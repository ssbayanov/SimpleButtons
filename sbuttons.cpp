...
bool AbstractButton::getPressState()
{
  return false;
}
...
SButton::SButton(uint8_t ID, uint8_t pin, int pressState, int pMode) : AbstractButton(ID)
{
  pinMode(pin, pMode);
  _buttonPin = pin;
...
SButton::SButton(uint8_t pin, int pressState, int pMode) : AbstractButton(0)
{
  pinMode(pin, pMode);
  _buttonPin = pin;
...
AButton::AButton(uint8_t ID, uint8_t pin, int minValue, int maxValue) : AbstractButton(ID)
{
  _minValue = minValue;
  _maxValue = maxValue;
...
AButton::AButton(uint8_t pin, int minValue, int maxValue) : AbstractButton(0)
{
  _minValue = minValue;
  _maxValue = maxValue;
...
#ifdef ESP32
TButton::TButton(uint8_t ID, uint8_t pin, int pressThreshold) : AbstractButton(ID)
{
...
TButton::TButton(uint8_t pin, int pressThreshold) : AbstractButton(0)
{
  _touchPort = pinToTouchPort(pin);
  _pressThreshold = pressThreshold;
  _lastValue = touchRead(_touchPort);
...
bool TButton::getPressState()
{
  ...
  Serial.print("Touch filtred value: ");
